...
PORT=3001 node src/index.js   # 用 3001 而非 7897 — 7897 常被 verge-mihomo 占用；同时需改 vite.config.js 中 proxy target 端口
```
...
2. `ontrack` 触发（视频轨）→ `_showScreenVideo()` 创建原始 DOM `<video>` 挂载到 `document.body`（`position: fixed`，`left: calc((100vw - 120px)/2)` 居中于中间区域，`max-width: calc(100vw - 520px)` 刚好填满，避开左右侧栏）
...
3. 黑色横杠 = CSS 固定宽度改为 `max-width: calc(100vw - 520px)` + `max-height: 75vh`
