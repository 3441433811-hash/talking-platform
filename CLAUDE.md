...
- 视频立即显示（黑色背景占位），`loadeddata` 事件到达后移除背景；3 秒兜底定时器
- 监听 track `mute`/`unmute` 事件诊断数据流：远程无数据时 mute=true
- `_currentScreenSharer` 字段追踪当前共享者，停止时按 userId 精确匹配清除 UI
...
### 屏幕共享 renegotiation 陷阱（重要！）
- **`createOffer({ iceRestart: false })`**：所有 renegotiation 必须显式禁用 ICE restart，否则浏览器可能生成新 ICE 凭据导致连接进入 failed 状态
- **`addTrack` 必须包在 try-catch 内**：WebRTC 规范要求 `signalingState === 'stable'` 才能调用，跨设备网络延迟可能导致抛 `InvalidStateError`
- **重协商前等 signalingState stable**：用 50ms 轮询等待 `pc.signalingState === 'stable'`
- **不要预创建 video transceiver**：`pc.addTransceiver('video', { direction: 'recvonly' })` 会改变初始 SDP 协商结构（多了 video m-line），反而干扰后续 renegotiation 的 ontrack 触发
- **`stopScreenShare` 保持同步**：`removeTrack` 后不要额外发 renegotiation offer（会触发不必要的 ICE 状态变化），直接 `removeTrack` + `track.stop()` 即可
- **ICE 状态处理**：只在 `connectionState === 'failed'` 时移除 peer。`disconnected` 可自动恢复；`iceConnectionState === 'failed'` 不应单独踢 peer
- **Server 信令用广播**：`socket.to(roomId).emit(...)` 简单可靠，不要尝试按 targetId 定向转发（`findSocketByUserId` 在不同 Socket.IO 版本下行为不一致）
- **`getDisplayMedia` 加分辨率约束**：`width/height: { ideal: 1920, max: 1920 }` 防止 TURN 中继带宽瓶颈
...
- **Git 注意**：仓库根目录是 `client/` 而非项目顶层，`git add` 路径相对 `client/`（如 `git add src/services/webrtc.js`）
...
### 屏幕共享看不到画面
1. **发送端检查**：`renegotiation offer sent to: X sktConnected: true sdpLines: N` — 确认 offer 发出
2. **接收端检查**：`复用已有 PC 处理重协商` + `ontrack: VIDEO` — 确认 renegotiation 到达且 ontrack 触发
3. **ICE 崩溃**：若 `连接状态: connecting → failed` → renegotiation 意外触发 ICE restart → 检查 `createOffer` 是否加了 `{ iceRestart: false }`
4. **视频不可见**：检查 DOM 中 `#screen-video-xxx` 元素是否存在；track 的 `mute` 事件表示无数据到达
